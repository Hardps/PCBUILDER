...
if(isset($_POST["submit"])){
  ...
  $maxram = $_POST["maxram"];
  $types = $_POST["types"];
  $duplicate = mysqli_query($conn, "SELECT * FROM motherboard WHERE (model = '$model' AND socket = '$socket' AND types = '$types')");
  if(mysqli_num_rows($duplicate) > 0){
    echo
    "<script> alert('product already added'); </script>";
  }
  else{
      $query = "INSERT INTO motherboard VALUES('','$brand', '$model','$socket','$maxram','$types')";
  ...
  <body  style="margin: 0% ; background-image:url('imgs/pcback.jpg') ">
      <header>
        ...
        <div class="bg">
            ...
            <br>
              <div>
                <a class="backtext"><b>add motherboard</b></a>
              </div>
            <br>
            <br>
            <div>
        ...
        <div >
            ...
            <br>
            <div class="input-box">
                <label class="input-label" style="color:white">max ram</label><br>
                <input type="text" id="maxram" name="maxram" class="input-1" onfocus="setFocus(true)" onblur="setFocus(false)" id="maxram" required value=""/>
            </div>
            <br>
            <div class="input-box">
                <label class="input-label" style="color:white">types</label><br>
                <input type="text" id="types" name="types" class="input-1" onfocus="setFocus(true)" onblur="setFocus(false)" id="types" required value=""/>
            </div>
            <br>
