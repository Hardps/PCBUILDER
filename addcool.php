...
if(isset($_POST["submit"])){
  ...
  $types = $_POST["types"];
  $size = $_POST["size"];
  $duplicate = mysqli_query($conn, "SELECT * FROM cool WHERE (model = '$model' AND types = '$types' AND size = '$size')");
  if(mysqli_num_rows($duplicate) > 0){
    echo
    "<script> alert('product already added'); </script>";
  }
  else{
      $query = "INSERT INTO cool VALUES('','$brand', '$model','$types','$size')";
...
<!DOCTYPE html>
<head>
    <title>Add Cooling System</title>
  ...
  <body  style="margin: 0% ; background-image:url('imgs/pcback.jpg') ">
      <header>
        ...
        <div class="bg">
            ...
            <br>
              <div>
                <a class="backtext"><b>add cooling system</b></a>
              </div>
            <br>
            <br>
            <div>
